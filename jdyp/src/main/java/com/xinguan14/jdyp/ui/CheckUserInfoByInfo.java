...
public class CheckUserInfoByInfo extends ParentWithNaviActivity implements View.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        initNaviView();
        //接收传递的user信息
         info= (BmobIMUserInfo)getBundle().getSerializable("userId");
        friendQuery();
        //构造聊天方的用户信息:传入用户id、用户名和用户头像三个参数
        info = new BmobIMUserInfo(info.getUserId(), info.getName(),info.getAvatar());
        dataBind();
    }
    public void dataBind(){
        //给控件绑定数据
        ImageLoader.getInstance().displayImage(info.getAvatar(),image_my_avatar, ImageLoadOptions.getOptions());
